...
class SuricateFDW(ForeignDataWrapper):
    ...
    def __init__(self, options, columns):
        ...
        if ELASTICSEARCH_VERSION[0] >= 7:
            ...
            self.arguments = {"index": self.index, "size": self.index, "explain":self.explain}
        else:
    ...
    def execute(self, quals, columns):
        ...
        try:
            ...
            log2pg("query_dict:({t},{l}, {v})".format(t=type(q_dict),l=len(q_dict), v=q_dict), logging.INFO)
            log2pg("pg_id:({t}, {v})".format(t=type(pg_id), v=pg_id),logging.INFO)
            if query:
                ...
                log2pg("response:({t},{l}, {v})".format(t=type(response),l=len(response), v=response), logging.INFO)
            else:
                response = self.client.search(
            ...
            while True:
                ...
                for result in response["hits"]["hits"]:
                    ...
                    yield self.format_out(result, pg_id=pg_id, query=query)
                log2pg('code arrived here- 139', logging.INFO)
